...
function getSessionDir(topic: string): string {
  return join(DATA_DIR, sanitizeTopicName(topic));
}

function getSessionFile(topic: string): string {
  return join(getSessionDir(topic), "session.json");
}

const memoryStore = new Map<string, LearningContext>();

export function createSession(sessionId: string, goal: string, topic?: string): LearningContext {
  ensureDataDir();
...
export function loadSessionByTopic(topic: string): { sessionId: string; context: LearningContext } | undefined {
...
function saveSession(sessionId: string, context: LearningContext): void {
...
export function listSessions(): { topic: string; updatedAt: string; currentStep: number }[] {
  ensureDataDir();
  const sessions: { topic: string; updatedAt: string; currentStep: number }[] = [];
    ...
    for (const dir of dirs) {
      if (dir.isDirectory()) {
        ...
        if (existsSync(sessionFile)) {
          try {
            const data = JSON.parse(readFileSync(sessionFile, "utf-8")) as LearningContext;
            sessions.push({
              topic: data.topic,
